import runpod
import torch
from transformers import SamModel, SamProcessor
from PIL import Image
import io
import base64
import numpy as np

# --- Global Model Cache ---
model = None
processor = None
device = "cuda" if torch.cuda.is_available() else "cpu"

def load_model():
    """Loads and caches the model and processor."""
    global model, processor
    if model is None or processor is None:
        model_id = "facebook/sam-vit-large"
        model = SamModel.from_pretrained(model_id).to(device)
        processor = SamProcessor.from_pretrained(model_id)
    return model, processor

def handler(job):
    """Processes segmentation and cutting jobs."""
    try:
        model, processor = load_model()
        job_input = job['input']
        
        # --- Input Validation ---
        if 'action' not in job_input:
            return {"error": "No action specified. Use 'segment' or 'cut'."}
        
        action = job_input['action']
        
        # Decode image
        try:
            image_data = base64.b64decode(job_input["image_base64"])
            pil_image = Image.open(io.BytesIO(image_data)).convert("RGB")
        except Exception as e:
            return {"error": f"Failed to decode image: {str(e)}"}

        # --- Segmentation ---
        if action == 'segment':
            if 'points' not in job_input:
                return {"error": "No points provided for segmentation."}
            
            points = job_input['points']
            img_width, img_height = pil_image.size
            
            # Prepare input points and labels
            input_points = [[[p["x"] * img_width, p["y"] * img_height] for p in points]]
            input_labels = [[p["label"] for p in points]]

            # Process and predict
            inputs = processor(
                pil_image,
                input_points=input_points,
                input_labels=input_labels,
                return_tensors="pt"
            ).to(device)
            
            with torch.no_grad():
                outputs = model(**inputs)
            
            # Move ALL tensors to CPU before post-processing
            masks = processor.image_processor.post_process_masks(
                outputs.pred_masks.cpu(),  # <-- Force CPU
                inputs["original_sizes"].cpu(),
                inputs["reshaped_input_sizes"].cpu(),
            )
            
            scores = outputs.iou_scores.cpu().numpy()[0]
            best_mask_idx = np.argmax(scores)
            
            # Double-check mask is on CPU before NumPy conversion
            best_mask = masks[0][0][best_mask_idx].cpu().numpy().astype(np.uint8)  # <-- .cpu() again for safety
            
            # Clean up CUDA memory
            del inputs, outputs
            torch.cuda.empty_cache()
            
            return {
                "mask": best_mask.flatten().tolist(),
                "scores": scores.tolist(),
                "width": best_mask.shape[1],
                "height": best_mask.shape[0],
            }

        # --- Cut Operation ---
        elif action == 'cut':
            if 'mask_base64' not in job_input:
                return {"error": "No mask data provided for cutting."}
            
            try:
                mask_data = base64.b64decode(job_input["mask_base64"])
                mask = Image.open(io.BytesIO(mask_data)).convert("L")
                
                rgba_image = pil_image.convert("RGBA")
                if rgba_image.size != mask.size:
                    mask = mask.resize(rgba_image.size, Image.NEAREST)
                
                cut_image = Image.new("RGBA", rgba_image.size, (0, 0, 0, 0))
                cut_image.paste(rgba_image, (0, 0), mask)
                
                byte_arr = io.BytesIO()
                cut_image.save(byte_arr, format='PNG')
                encoded_string = base64.b64encode(byte_arr.getvalue()).decode('utf-8')
                
                return {"image_base64": encoded_string}
            
            except Exception as e:
                return {"error": f"Cut operation failed: {str(e)}"}
        
        else:
            return {"error": f"Invalid action: {action}"}
    
    except Exception as e:
        return {"error": f"Unexpected error: {str(e)}"}

runpod.serverless.start({"handler": handler})
